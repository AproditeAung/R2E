...
@section('genre_index_active','mm-active')
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('genre.update',$genre->id) }}" method="post" >
                            @method('PUT') @csrf
                            <input type="text" name="genre" value="{{ old('genre',$genre->name) }}" class="form-control">
                        ...
                        <table class="table table-striped  ">
                            ...
                            @foreach($Genres as $g)
                                <tr>
                                    ...
                                    <td class="d-flex ">
                                        <a href="{{ route('genre.edit',$g->id) }}" class="pe-7s-pen text-decoration-none btn btn-sm  fsize-1 "></a>
                                        <form id="deleteGenre" action="{{ route('genre.destroy',$g->id) }}" method="post" class="mx-2  ">
                                            @csrf @method('DELETE')
                                        </form>
