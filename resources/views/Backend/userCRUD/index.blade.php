                    ...
                    <tbody>
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td class=" ">
                                    <form action="{{ route('user.upgradeAdmin') }}" method="post" class="d-flex justify-content-between align-items-center ">
                                        @csrf
                                        <span class="{{ $user->is_premium == 1 ? 'pe-7s-check ' : 'pe-7s-attention  ' }} h3  "></span>
                                        <div class="form-group  ">
                                            <div class="form-check form-switch">
                                                <input type="hidden" value="{{ $user->id }}" name="user_id">
                                                <input class="form-check-input" {{  $user->is_premium == 1 ? 'checked' : '' }} name="admin_upgrade" onchange="this.form.submit()" type="checkbox" role="switch" id="flexSwitchCheckChecked" >
                                            </div>
                                        </div>
                                    </form>
                                </td>
